/**
 * layout.js — squarified treemap + recursive "city" layout.
 *
 ...
 *
 * @param {object} root       tree root (folder node)
 * @param {object} [opts]
 * @param {number} [opts.size=900]  world extent of the root plate (square)
 * @returns {object} the same root
 */
export function layoutCity(root, opts = {}) {
 ...
 * @param {{x:number,z:number,w:number,h:number}} rect
 * @returns {Array<{item:object,index:number,x:number,z:number,w:number,h:number}>}
 */
export function treemap(items, rect) {
  ...
  for (let i = 0; i < items.length; i++) {
    const weight = Math.max(Number(items[i].weight) || 0, 0);
    list.push({ item: items[i], index: i, weight });
  }
  let total = 0;
...
export function buildingHeight(loc) {
...
function layoutModules(fileNode, rect) {
  ...
  for (const cell of cells) {
    ...
    plots.push({ mod: mods[cell.index], x: r.x, z: r.z, w: r.w, h: r.h });
  }
  return plots;
}
...
function squarify(entries, rect, out) {
  let i = 0;
...
function worstRatio(rowArea, min, max, short) {
