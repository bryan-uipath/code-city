/**
 * labels.js — map-style dynamic labeling.
 *
 ...
 *
 * Candidates are plain records supplied by main.js:
 *   { key, text, tier: 'folder'|'file'|'module', pos: Vector3, size: number }
 */
...
const TIER_PX = { folder: 26, file: 18, module: 14 };
const TIER_COLOR = { folder: '#bdf3ff', file: '#7fd8ea', module: '#9ae9ff' };

export function createLabeler(scene, camera) {
  const group = new THREE.Group();
  group.name = 'labels';
  scene.add(group);

  const cache = new Map();     // key -> { sprite, tier, text }
  const active = new Set();    // keys currently chosen
  let candidates = [];
  let acc = PICK_INTERVAL;

  const _v = new THREE.Vector3();
  const boxes = [];            // reused screen-space rejection boxes
  ...
  function overlaps(x, y, w, h) {
    for (let i = 0; i < boxes.length; i += 4) {
      if (x < boxes[i] + boxes[i + 2] && x + w > boxes[i] && y < boxes[i + 1] + boxes[i + 3] && y + h > boxes[i + 1]) {
        return true;
      }
    }
    return false;
  ...
  function show(c, dist, tanHalf, vh) {
