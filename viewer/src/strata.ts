  ...
  function update(range: { start: number; cursor: number | null }, opts?: StrataUpdate): void {
    ...
    for (const stack of stacks) {
      ...
      for (const lv of levels) {
        ...
        pos.set(cx, top + level * levelHeight * rise, cz);
        scale.set(baseW * k, slabHeight * rise, baseH * k);
        m.compose(pos, q, scale);
        mesh.setMatrixAt(n, m);
        ages[n] = lv.age;
        records[n] = { file: node, commit: lv.commit, band: null, level };
        n++;
        level++;
      }
      ...
      if (level === 0 && n < capacity) {
        ...
        pos.set(cx, top, cz);
        scale.set(baseW, stubHeight * rise, baseH);
        m.compose(pos, q, scale);
        mesh.setMatrixAt(n, m);
        ages[n] = 0;
        records[n] = { file: node, commit: null, band: null, level: 0 };
